...
# In this problem set you will work with the Star Wars API. We recommend you read through some of the documentation (https://swapi.co/documentation) and 
# reference it as you work through the following problems. 
# You will also want to reference the requests documentation (https://requests.readthedocs.io/en/master/user/quickstart/#quickstart).
...
for film in films_dict['results']: #loops through films within the outer dictionary
    temp_tup = (film['title'],film['episode_id'], len(film['characters'])) #creates a contained temp tuple 
    title_id_chars.append(temp_tup) #appends our tuple to our list
    
#print(title_id_chars)
#print(films_dict['results'][4]['title'])
...
# BEGIN PROBLEM 3
def call_API(category, id = None):
    """grabs a category and an id and performs a search of our API"""
    baseurl = 'https://swapi.co/api'
    if id != None:
        return requests.get(baseurl + "/" + category + "/" + str(id) + "/").json()
    else:
        return requests.get(baseurl + "/" + category + "/").json()
...
def create_new_record(record, keys):
    """makes a new dictionary out of a given record"""
    new_record = {} # sets up a new dictionary within our function
    for key, value in record.items(): # makes the dict a list so that we can loop through
        if key in keys: # if there is a key, which there should be because "keys" is a required argument of our function...(see r2d2_keys below)
            new_record[key] = value # we create a new key and assign it a value
    return new_record # returns our completed dictionary



r2d2_keys = ("name", "height", "mass", "hair_color", "skin_color", "eye_color", "birth_year", "gender") # a tuple of our desired keys to be assigned in our "new record"
search_dict = requests.get(search_url).json() # makes a variable for our request pull from the API

for item in search_dict['results']: # here we loop through the contents of the "results" for the "r2_d2" dict. 
    r2_d2 = create_new_record(item, r2d2_keys) # we apply our function to the empty r2_d2 dict created at the top of the prob. this allows us to "filter" through all items and apply just our selected keys.
...
def call_API_updated(category, option = None): # set up our function
    """API call that takes a category of a dict and an "option" of an inner-dict"""
    params = {'search':option} # params sets up our "?search=" query for us and handles URL encoding
    if type(option) is str: 
        dictionary = requests.get(f"https://swapi.co/api/{category}", params=params).json() # if the option argument is a string we need that "params" encoding to save our asses if there are spaces
    if type(option) is int:
        dictionary = requests.get(f"https://swapi.co/api/{category}/{str(option)}").json() # if the option argument is an int we need to convert it to a string for the function to work
    if option is None:
        dictionary = requests.get(f"https://swapi.co/api/{category}").json() # if there is no option input we have it set as a default variable of "None", which takes us just to the category
    return dictionary
...
r2d2_films = search_dict['results'][0]['films'] # find all films that r2d2 appears in

for film in r2d2_films: # loop through "films" (they're actually urls that represent the films)
    reference = film.split("/")[-3] # split and isolate the string <film> for when we apply it to our function
    id = film.split("/")[-2] # do the same for each film "number"
    id = int(id) # convert the film "number" to an int (it's a string before our conversion)
    name = call_API_updated(reference, id)['title'] # call our function to find each film dictionary. access "title" key to produce just the film titles
    films.append(name) # append titles to our empty list

r2_d2['films'] = films # assign our list to a new key in the r2_d2 dict

with open('r2_d2.json', 'w') as file_obj: # prepare the dict as a json file
    json.dump(r2_d2, file_obj, indent = 2) # write the object to our directory
